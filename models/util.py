...
def cal_lstm_crf_loss(crf_scores, targets, tag2id):
    ...
    # import pdb
    # pdb.set_trace()
    targets = targets.masked_select(mask)  # shape [real_L], 直接平铺所有的
    # crf_scores :batch_size,seq_len,out_size,out_size
...
def indexed(targets, tagset_size, start_id):
    ...
    for col in range(max_len - 1, 0, -1):
        targets[:, col] += (targets[:, col - 1] * tagset_size)  # 因为是从下一个标签是从前一个标签转移过来的，所以这里是为了映射转移矩阵
