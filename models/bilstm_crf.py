...
class BiLSTM_CRF(nn.Module):
    ...
    def test(self, test_sents_tensor, lengths, tag2id):
        ...
        for step in range(L - 1, 0, -1):
            batch_size_t = (lengths > step).sum().item()
            if step == L - 1:
                # step * tagset_size 这样做是为了方便后面锁定<end>标签对应的id
                index = torch.ones(batch_size_t).long() * (step * tagset_size)
                index = index.to(device)
                index += end_id  # 这里在原来的基础上加上end_id就可以锁定backpointer中<end>标签对应的id
            else:
                prev_batch_size_t = len(tags_t)
                new_in_batch = torch.LongTensor(
                    [end_id] * (batch_size_t - prev_batch_size_t)).to(
                    device)  # 因为每个batch中的句子是按照长度来排序的（由长到短），按照批量回溯，有可能发现非padding部分的句子，这里要计算出新发现的句子的end_id
                offset = torch.cat(
                    [tags_t, new_in_batch],
                    dim=0
                )  # 这个offset实际上就是前一时刻的
                ...
                index = index.to(device)
                index += offset.long()
            try:  # tags_t.shape:batch_size, 1  这里找出end_id对应的前一个标记
                tags_t = backpointer[:batch_size_t].gather(
                    dim=1, index=index.unsqueeze(1).long())
            except RuntimeError:
                import pdb
                pdb.set_trace()
            tags_t = tags_t.squeeze(1)  # 把tags_t最后一维压缩
